...
const initApp = () => {
    ...
    const initPlatformPulse = () => {
        const rootNode = document.querySelector('[data-platform-pulse]');
        if (!rootNode) {
            return;
        }

        const onlineNode = rootNode.querySelector('[data-platform-online]');
        const openRoomsNode = rootNode.querySelector('[data-platform-open-rooms]');
        const totalUsersNode = rootNode.querySelector('[data-platform-total-users]');
        const updatedNode = rootNode.querySelector('[data-platform-updated]');
        const chartNode = rootNode.querySelector('[data-platform-chart]');
        const rangeButtons = Array.from(rootNode.querySelectorAll('[data-platform-range]'));
        if (!onlineNode || !openRoomsNode || !totalUsersNode || !updatedNode || !chartNode || !rangeButtons.length) {
            return;
        }

        const storageKey = 'mycubePlatformPulseHistoryV1';
        const maxHistoryPoints = 720;
        let selectedRange = Number(rangeButtons.find((button) => button.classList.contains('is-active'))?.dataset.platformRange || 300);
            ...
            try {
                ...
                return parsed
                    ...
                    .map((item) => ({
                        ...
                        openRooms: Number(item.openRooms) || 0,
        ...
        const toPolyline = (series, toX, toY) => series.map((point, index) => `${toX(index)},${toY(point)}`).join(' ');

        const renderChart = () => {
            rangeButtons.forEach((button) => {
                button.classList.toggle('is-active', Number(button.dataset.platformRange) === selectedRange);
            });

            const nowTs = Date.now();
            const visible = history.filter((item) => nowTs - item.ts <= selectedRange * 1000);

            if (!visible.length) {
                chartNode.innerHTML = '<p class="record-history__empty">Недостаточно данных для графика. Подождите несколько обновлений.</p>';
                return;
            }
            ...
            const onlineValues = visible.map((item) => item.online);
            const roomValues = visible.map((item) => item.openRooms);
            const allValues = onlineValues.concat(roomValues);
            ...
            const toY = (value) => padTop + chartHeight - (value / maxValue) * chartHeight;

            const gridTicks = [0, 0.33, 0.66, 1];
            const grid = gridTicks.map((ratio) => {
                const y = padTop + chartHeight - ratio * chartHeight;
                return `<line class="platform-pulse__grid" x1="${padLeft}" y1="${y}" x2="${width - padRight}" y2="${y}" />`;
            }).join('');

            const yLabels = gridTicks.map((ratio) => {
                const y = padTop + chartHeight - ratio * chartHeight;
                const label = Math.round(maxValue * ratio);
                return `<text class="platform-pulse__axis" x="${padLeft - 8}" y="${y}" text-anchor="end" dominant-baseline="middle">${label}</text>`;
            }).join('');

            const xLabels = [0, 0.5, 1].map((ratio) => {
                const x = padLeft + ratio * chartWidth;
                const index = Math.min(visible.length - 1, Math.max(0, Math.round(ratio * (visible.length - 1))));
                const date = new Date(visible[index].ts);
                const text = date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' });
                return `<text class="platform-pulse__axis" x="${x}" y="${height - 16}" text-anchor="middle">${text}</text>`;
            }).join('');

            const onlineLine = toPolyline(onlineValues, toX, toY);
            const roomsLine = toPolyline(roomValues, toX, toY);
            const baselineY = padTop + chartHeight;
            const areaPath = `M ${toX(0)} ${baselineY} L ${onlineLine} L ${toX(visible.length - 1)} ${baselineY} Z`;

            chartNode.innerHTML = `
                <svg class="platform-pulse__svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="График онлайн и открытых комнат">
                    ${grid}
                    ${yLabels}
                    <path class="platform-pulse__area" d="${areaPath}"></path>
                    <polyline class="platform-pulse__line platform-pulse__line--online" points="${onlineLine}"></polyline>
                    <polyline class="platform-pulse__line platform-pulse__line--rooms" points="${roomsLine}"></polyline>
                    ${xLabels}
                </svg>
            `;
        };
        ...
        const applyLatest = () => {
            ...
            onlineNode.textContent = formatNumber(latest.online);
            openRoomsNode.textContent = formatNumber(latest.openRooms);
            totalUsersNode.textContent = formatNumber(latest.totalUsers);
            updatedNode.textContent = formatStamp(latest.ts);
        };
        ...
        const syncStats = async () => {
            try {
                ...
                history.push({
                    ts: Date.now(),
                    online: Number(payload.online_users) || 0,
                    openRooms: Number(payload.open_rooms) || 0,
        ...
        rangeButtons.forEach((button) => {
            button.addEventListener('click', () => {
                selectedRange = Number(button.dataset.platformRange || 300);
                renderChart();
            });
        });
